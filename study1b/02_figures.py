"""
HRIT VPM Study 1b — Step 2: Publication Figures
================================================
Reads results/pews_subject_results.csv and generates three figures.

Figure 1  Group-level PEWS — mean ± SE of normalised PE and its rolling
          variance, aligned to N3 onset. Variance peak annotated.
...
Output: figures/fig1_pews_group.png
        figures/fig2_pews_lags.png
...
results_path = os.path.join(RESULTS_DIR, 'pews_subject_results.csv')
if not os.path.exists(results_path):
    sys.exit(f"Results not found: {results_path}\nRun 01_pews_analysis.py first.")
...
# ── Figure 1: Group-level PEWS ────────────────────────────────────────────────

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(8.5, 3.8))

# Panel A — rolling mean (PE falls into N3)
ax1.fill_between(t_sec, gm - se_m, gm + se_m, color=LPURP, alpha=0.55)
ax1.plot(t_sec, gm, color=PURPLE, lw=2.0, label='Permutation entropy (mean)')
...
ax1.set_ylabel('Normalised permutation entropy')
ax1.set_title('A.  Mean: falls into N3', loc='left', fontweight='bold', pad=5)
ax1.legend(fontsize=8, frameon=False)

# Panel B — rolling variance with annotated peak
ax2.fill_between(t_sec, gv - se_v, gv + se_v, color='#f9c48c', alpha=0.55)
ax2.plot(t_sec, gv, color=ORANGE, lw=2.0, label='Permutation entropy (variance)')
...
fig.suptitle(
    'Figure 1.  Study 1b — PEWS Variance-Precedes-Mean (Permutation Entropy)\n'
    f'Group mean ± SE, N = {n} subjects. '
    f'Pre-registration: doi.org/10.17605/OSF.IO/EYMSF',
    fontsize=9.5, fontweight='bold', x=0.02, ha='left')
plt.tight_layout(rect=[0, 0, 1, 0.9])
path1 = os.path.join(FIGURES_DIR, 'fig1_pews_group.png')
plt.savefig(path1)
print(f'✓  {path1}')
plt.close()
...
# Panel A — histogram
ax1.axvline(0, color=RED, lw=1.2, ls='--', alpha=0.8, label='Boundary (lag = 0)')
ax1.axvline(lags.mean(), color=PURPLE, lw=1.5,
            label=f'Mean = {lags.mean():+.0f}s ({lags.mean()/60:.1f} min)')
ax1.hist(lags, bins=max(6, len(lags) // 3),
         color=LPURP, edgecolor=PURPLE, lw=0.8, alpha=0.85)
...
ax2.axvline(lags.mean(), color=PURPLE, lw=1.2, ls=':',
            label=f'Mean lag ({lags.mean():+.0f}s)')
ax2.set_xlabel('Time relative to N3 onset (s)')
ax2.set_ylabel('Normalised variance')
ax2.set_title(f'B.  Individual traces (N = {len(raw_ts)})',
              loc='left', fontweight='bold', pad=5)
ax2.legend(fontsize=8, frameon=False)

fig.suptitle(
    'Figure 2.  Study 1b — Individual PEWS Lags (Permutation Entropy)\n'
    'Each grey trace = one subject.',
    fontsize=9.5, fontweight='bold', x=0.02, ha='left')
plt.tight_layout(rect=[0, 0, 1, 0.93])
path2 = os.path.join(FIGURES_DIR, 'fig2_pews_lags.png')
plt.savefig(path2)
print(f'✓  {path2}')
plt.close()

# ── Figure 3: Power-law β ─────────────────────────────────────────────────────
...
if n_b < 3:
    ...
    ax.axvline(0.546, color=RED,    lw=1.5, ls='--', label='HRIT predicted: β = 0.546')
    ax.axvline(0.500, color=GREY,   lw=0.8, ls=':', label='Exact exponent: β = 0.500')
    ...
    ax.set_title(f'Figure 3.  Study 1b — H3: Power-Law Exponent (Exploratory)\n'
...
print('Run next:  python 03_write_results.py')
